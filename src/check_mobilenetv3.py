...
print("=" * 60)
print("MOBILENETV3-SMALL DIAGNOSTIC CHECK")
print("=" * 60)
...
model = MobileNetV3SmallCrowdCounter().to(DEVICE)
...
with torch.no_grad():
    ...
    for index in range(number_of_samples):
        ...
        minimum_value = (
            prediction_cpu.min().item()
        )

        maximum_value = (
            prediction_cpu.max().item()
        )

        mean_value = (
            prediction_cpu.mean().item()
        )

        standard_deviation = (
            prediction_cpu.std().item()
        )

        density_sum = (
            prediction_cpu.sum().item()
        )
...
print()
print("=" * 60)
print("MOBILENETV3-SMALL DIAGNOSTIC COMPLETED")
print("=" * 60)